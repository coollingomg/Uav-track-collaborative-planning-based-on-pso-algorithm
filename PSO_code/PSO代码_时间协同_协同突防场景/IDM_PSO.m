function [Gbest, fitness_beat_iters] = IDM_PSO(startPos,goalPos,menaceParams,mm)
% -------------------------------------------------------------
% 改进的多种群粒子群算法
% -------------------------------------------------------------
    % 所有参数定义到params结构体中
    params = createParams(startPos, goalPos, menaceParams, mm);
...
function params = createParams(startPos, goalPos, menaceParams, mm)
% -------------------------------------------------------------
% 创建参数结构体
% -------------------------------------------------------------
    ...
    params.sizepop = 90;                             % 粒子数量
    params.subsize = params.sizepop / 3;             % 子族群粒子数量
    params.maxgen = 700;                             % 迭代次数

    % 粒子速度界限
    params.velocityLimits = [20 * 10^3, 20 * 10^3, 100];  % 定义每个维度的速度界限
    ...
    % 粒子位置界限
    params.mapRange = [200 * 10^3, 200 * 10^3, 1000];
