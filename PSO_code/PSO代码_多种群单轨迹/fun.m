function fitness = fun(pop, startPos, goalPos, X, Y, Z, menace, S, D, M, g, dim, menace_obj_R, mapRange, Bmax, Umax)
% -------------------------------------------------------------
...
if flag == 0
    ...
    fitness1 = sum(sqrt(dx.^2 + dy.^2 + dz.^2))*0.45;
    % 威胁代价
    fitness2 = sum(sum(T))*0.3;
    % 高度代价：频繁的调整飞行高度不仅会增加能源消耗，同时也不利于飞行的安全，因此希望无人机在飞行过程中尽可能保持平稳
    fitness3 = sum(M*g*sqrt(dz.^2))*0.25;     % M 为无人机总质量， g 为重力加速度，dz 为前后两个航迹点的高度差
