...
for i=1:maxgen
    ...
    for j=1:sizepop
        ...
        if sum(j == top_swarm) == 1
            ...
            f1 = fun(X1,startPos,goalPos,X,Y,Z,menace,S,D,M,g,dim,menace_obj_R,mapRange,Bmax,Umax);
        ...
        elseif sum(j == bot_swarm) == 1
            ...
            if rand > (1/2*(1+atan(i/maxgen)*4/pi))         % 变异概率判断，不是每次都变异
                ...
                % 更新变异后的适应度
                fNEW = fun(X1,startPos,goalPos,X,Y,Z,menace,S,D,M,g,dim,menace_obj_R,mapRange,Bmax,Umax);
...
%% 结果分析
% 三维、二维图
Draw_result_path(startPos,goalPos,X,Y,Z,Gbest,dim,menaceParams)

% 画适应度迭代图
figure('Name','最佳适应度变化图');
plot(fitness_beat_iters,'Color',[021 151 165]/255,'LineWidth',3)
...
---------------------------------------------------------------
zbest3：算法得到的最佳路径或最佳解
yy3：算法每一代的最优适应度值组成的向量
%}
yy3  = fitness_beat_iters;
zbest3 = Gbest;
save IDM_PSO.mat zbest3 yy3
