...
% ----------------------------------------------这一部分的插值没有意义，返回的都是插值原数据，没有进行预测------------------------------------------------
% % 构造曲面网格，并转化为列向量
% [x , y] = meshgrid(1:mapRange(1), 1:mapRange(2));
% x = reshape(x, mapRange(1) * mapRange(2), 1);
% y = reshape(y, mapRange(1) * mapRange(2), 1);
% peakData = reshape(peakData,length(peakData(:)),1);
% 
% % 使用griddata函数进行插值。这个函数通过输入的 x、y 坐标和相应的数据值，在新的坐标网格上进行插值计算
% [X,Y,Z] = griddata(x,y,peakData,...
%                    linspace(min(x),max(x),mapRange(1))',...
%                    linspace(min(y),max(y),mapRange(2)));
% ----------------------------------------------这一部分的插值没有意义，返回的都是插值原数据，没有进行预测------------------------------------------------
