...
% -------------------------------------------------------------
    ...
    k = length(x_seq);
    i_seq = linspace(0,100,k);
    interp = linspace(0,100,100);
    ...
    for i = 1:size(X_seq, 2)
        ...
        if z < z_interp                           % 如果平滑路径中当前点的 z 坐标小于插值高度（即在地形表面下方）
    ...
    if flag == 0
        ...
        fitness1 = sum(sqrt(dx.^2 + dy.^2 + dz.^2))*0.45;       % 路径约束
        fitness2 = sum(sum(T))*0.3;                             % 威胁代价
        fitness3 = sum(M*g*sqrt(dz.^2))*0.25;                   % 高度代价
